...
import { INFIELD_BUTTON_VALID_SIZES } from '@spectrum-web-components/core/components/infield-button';
...
export const AnatomyTest: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('each Anatomy button has an aria-label on its inner button', async () => {
      for (const btn of buttons) {
    ...
    await step(
      ...
      async () => {
        expect(
          ...
          'default button disabled is false'
        ).toBe(false);
        expect(defaultButton.quiet, 'default button quiet is false').toBe(
          false
        );
...
export const AccessibilityTest: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('internal button has no aria-pressed or aria-expanded', async () => {
      const internalButton = button.renderRoot.querySelector('button');
      expect(
...
export const IconOnlyMissingLabelWarningTest: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      'warns when icon-only button is missing accessible-label',
      () =>
