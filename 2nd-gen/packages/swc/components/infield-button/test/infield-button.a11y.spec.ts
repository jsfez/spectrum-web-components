...
test.describe('Infield Button - ARIA Snapshots', () => {
  ...
  test('should have no role attribute on the host element', async ({ page }) => {
    const root = await gotoStory(
      page,
      'components-infield-button--overview',
      'swc-infield-button'
...
test.describe('Infield Button - Keyboard Interactions', () => {
  ...
  test('inner button should not be in tab order — tabindex="-1"', async ({
    ...
    const isInfieldFocused = await page.evaluate(() => {
      const active = document.activeElement;
      return active instanceof HTMLElement && active.tagName.toLowerCase() === 'swc-infield-button';
    });
    expect(
      isInfieldFocused,
      'swc-infield-button is not in the tab order'
    ).toBe(false);
  });
});
